...
const AssignedPage = () => {
    ...
    const [incidentFilter, setIncidentFilter] = useState(''); // State for incident filter
    const [addressFilter, setAddressFilter] = useState('');   // State for address filter
    const [barangays, setBarangays] = useState<string[]>([]); // Barangays list from JSON
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage, setItemsPerPage] = useState(10);
    const router = useRouter();

    // Fetching barangay data from the public directory
    useEffect(() => {
    ...
    // Fetching assigned clients from Firestore
    useEffect(() => {
    ...
    // Filter based on search query, incident type, and address
    useEffect(() => {
        const result = clients.filter(client =>
            ...
            (addressFilter === '' || client.formData.address === addressFilter)
        );
