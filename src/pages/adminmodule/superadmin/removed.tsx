...
const RemovedPage = () => {
    ...
    const [barangays, setBarangays] = useState<string[]>([]); // To store barangay list
    const [addressFilter, setAddressFilter] = useState(''); // To filter by address
    ...
    const fetchRemovedClients = async () => {
        try {
            ...
            setError('Failed to fetch removed clients.');
            console.error(err);
        }
    };

    useEffect(() => {
        // Apply search and incident type filters
        let result = clients.filter(client =>
            client.formData.name.toLowerCase().includes(searchQuery.toLowerCase())
        );

        if (incidentFilter) {
            result = result.filter(client => client.formData.incidenttype === incidentFilter);
        }

        if (addressFilter) {
            result = result.filter(client => client.formData.address === addressFilter);
        }

        setFilteredClients(result);
        setCurrentPage(1); // Reset to first page on search/filter
    }, [searchQuery, incidentFilter, addressFilter, clients]);
    ...
    return (
        <div className="p-8 rounded bg-white">
            <h1 className="text-lg font-bold mb-4">Removed Incidents</h1>
