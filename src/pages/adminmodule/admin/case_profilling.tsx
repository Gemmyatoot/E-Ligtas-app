...
const CaseProfillingPage: React.FC = () => {
    const { userType } = useUser(); // Assuming the user email is part of your context
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (!user) {
                ...
                setLoading(false);
                fetchCases(user.email); // Pass the email of the logged-in user
            }
        });
    ...
    const fetchCases = async (userEmail: string | null) => {
        setLoading(true);
        try {
            const casesQuery = query(
                ...
                limit(50) // Fetch a maximum of 50 records
            );

            const querySnapshot = await getDocs(casesQuery);
            const casesData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Case[];

            // Filter cases to show only those assigned to the logged-in user
            const filteredCases = casesData.filter(caseItem => caseItem.assigned === userEmail);
    ...
    const paginateCases = () => {
        const sortedCases = [...cases].sort((a, b) => {
            ...
            // Neglect cases go to the bottom
    ...
    const generateExcel = () => {
        ...
        const wscols = Object.values(columnWidths).map(length => ({ wch: length + 2 })); // Adding some padding to the width
    ...
    return (
        <div className="p-10 bg-white shadow-lg rounded-lg">
            ...
            {/* Search Input and Generate Excel Button */}
            <div className="flex justify-between mb-5">
                <input
                    ...
                    className="border border-gray-300 rounded-lg p-2 w-80 text-sm"
                />
                <button
...
export default CaseProfillingPage;
