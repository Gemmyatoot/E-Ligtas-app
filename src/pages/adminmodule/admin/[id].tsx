...
const ChatPage: React.FC = () => {
    ...
    // Initialize user ID from session storage
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchUserStatus = () => {
            if (!id || typeof id !== 'string') {
                console.error("User ID is null, undefined, or not a string");
                return;
            }

            try {
                // Real-time listener for the user's status document from Firestore
                const userDocRef = doc(db, 'users', id); // Assuming 'users' collection and id document
                ...
                // Cleanup the listener when the component is unmounted or id changes
                return unsubscribe;
    ...
    const handleSendMessage = async () => {
        ...
        if (!id || (Array.isArray(id) ? id.length === 0 : !id) || (!newMessage && images.length === 0)) return;
    
        setIsSending(true);
        try {
            // Ensure id is a string before proceeding
            const userDocRef = doc(db, 'users', Array.isArray(id) ? id[0] : id); // Use the first id if it's an array
            const chatsCollectionRef = collection(userDocRef, 'chats');
    
            // Upload images if any
            const uploadedImageUrls: string[] = [];
            for (const { file } of images) {
                const fileRef = ref(storage, `users/${id}/chats/${file.name}`);
    ...
    return (
        <div className="flex h-screen bg-gray-100" style={{ height: 'calc(100vh - 85px)' }}>
            {/* Chat Section */}
            <div className="relative flex flex-col w-2/3 bg-white shadow-md">
                <div className="p-4 bg-blue-600 text-white text-sm font-medium">
                    <p className="text-sm">Report ID: {id?.slice(0, 3)}{id?.slice(-3)}</p>
                </div>
                <div
                ...
                >
                    {messages.map((message) => (
                        <div
                            key={message.id}
                            className={`mb-4 p-3 rounded-lg max-w-[75%] ${message.sentBy === id
                                ? 'mr-auto bg-gray-200' // Sent by current user (right-aligned)
                                : 'ml-auto bg-blue-100' // Sent by others (left-aligned)
                                }`}
                        >
                            {message.type === 'chat' && (
            ...
            <div className="flex flex-col w-1/3 bg-gray-200 p-4">
                <div className='flex justify-between mb-3 font-medium'>
                    Report Information

                    <button title='Download Data' className='rounded bg-blue-700 p-2'><svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="bg-white shadow p-4 mt-3 overflow-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
                    {userData && (
                        <div className="text-xs">
                            <div className='rounded shadow bg-yellow-500 p-2 text-white font-medium my-2'>{userData.formData?.incidenttype}</div>
                            <p>Reportee: {userData.formData?.reportee}</p>
                            <p>Phone Number: {userData.formData?.reporteenum}</p>
                            <p>Victim's Name: {userData.formData?.name}</p>
                            <p>Incident Type: {userData.formData?.incidenttype}</p>
